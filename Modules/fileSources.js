import { SmartSource, KeyValue, MetaData, WebPage } from "./resources.js";
...
var files = {
    'blank': new SmartSource(['DRIVE://1TNliEBLg0rNdgu1IMyKW5qSdXlhQ2qH7']),
};

var sources = {};
var google_drive_ids = [
...
var webPages = {
    'TOTW #1': new WebPage('TOTW #1', new MetaData ([
        ['link', 'TOTW_1'],
        ['next', 'TOTW #2'],
    ])),
    'TOTW #2': new WebPage('TOTW #2', new MetaData ([
        ['link', 'TOTW_2'],
        ['previous', 'TOTW #1'],
        ['next', 'TOTW #3'],
    ])),
    'TOTW #3': new WebPage('TOTW #3', new MetaData ([
        ['link', 'TOTW_3'],
        ['previous', 'TOTW #2'],
        ['next', 'TOTW #4'],
    ])),
...
for (let chapter = 1; chapter <= 21; chapter++) {
    files['TOTW #' + chapter + ' - Cover'] = new SmartSource();
    switch (chapter) {
        default:
            for (let page = 1; page <= 5; page++) {
                files['TOTW #' + chapter + ' - Pg.' + page] = new SmartSource();
            };
            break;
        case 9:
...
google_drive_ids.forEach(folder => {
    for (let i = 0; i < folder.content.length; i++) {
        if (i == 0) {
            files[folder.title + ' - Cover'].addSources(['DRIVE://' + folder.content[i]]);
        } else {
            files[folder.title + ' - Pg.' + i].addSources(['DRIVE://' + folder.content[i]]);
        };
    };
});
...
// Exports
export { chapterLengths, files, webPages };